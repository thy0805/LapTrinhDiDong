...
import 'package:nutrifit/modules/sleep/views/alarm_ring_screen.dart';
...
class SleepController extends GetxController {
  ...
  void onInit() {
    ...
    // Trì hoãn đồng bộ 5 giây để app load xong giao diện, tránh treo máy nhen Thy
    Future.delayed(const Duration(seconds: 5), () {
      syncWithHealthFit(showNotification: false);
    });

    Timer.periodic(const Duration(minutes: 10), (timer) {
      // Chỉ tự động đồng bộ nếu đã được cấp quyền trước đó
      ...
      if (s['bedtime'] != null) {
        try {
          ...
          if (scheduledForToday.isBefore(now)) {
            scheduledForToday = scheduledForToday.add(const Duration(days: 1));
          }
  ...
  void _listenToAlarmRing() {
    _alarmSubscription = Alarm.ringing.listen((alarmSet) {
      if (alarmSet.alarms.isNotEmpty) {
        Get.to(() => AlarmRingScreen(alarmSettings: alarmSet.alarms.first));
      }
    });
  }
  ...
  void _calculateAnalytics() {
    ...
    for (int i = 0; i < 7; i++) {
      DateTime date = today.subtract(Duration(days: 6 - i));
      labels[i] =
          "${_getVietnameseDay(date.weekday)}\n${date.day}/${date.month}";
    }
    ...
    if (sleepLogs.isNotEmpty) {
      ...
      if (sortedLogs.isNotEmpty) {
        ...
        lastNightSleep.value = (sortedLogs.first['duration'] ?? 0.0).toDouble();

        if (sortedLogs.length >= 2) {
          double yesterdaySleep = (sortedLogs[1]['duration'] ?? 0.0).toDouble();
          if (yesterdaySleep > 0) {
            sleepTrend.value =
    ...
    for (var log in sleepLogs) {
      try {
        ...
        int difference = today.difference(logDay).inDays;

        if (difference >= 0 && difference < 7) {
          weekData[6 - difference] += (log['duration'] ?? 0.0).toDouble();
        }

        if (difference >= 0 && difference < 7) {
          debt +=
              (targetSleepHours.value - (log['duration'] ?? 0.0).toDouble());
        }
      } catch (e) {
  ...
  Future<void> setBedtimeReminder(int reminderId, DateTime bedtime) async {
    DateTime reminderTime = bedtime.subtract(const Duration(minutes: 15));
    ...
    final reminderSettings = AlarmSettings(
      ...
      notificationSettings: NotificationSettings(
        ...
        body:
            'Còn 15 phút nữa là đến giờ ngủ. Bạn nên chuẩn bị lên giường nhé!',
      ),
    );
  ...
  Future<void> syncWithHealthFit({bool showNotification = true}) async {
    try {
      if (showNotification) {
        Get.snackbar(
          'Đang đồng bộ',
      ...
      if (!_healthService.isAuthorized.value) {
        debugPrint("--- SleepController: Chưa có quyền Health, đang thử xin lại... ---");
      ...
      if (_healthService.isAuthorized.value) {
        DateTime now = DateTime.now();
        DateTime yesterday = now.subtract(const Duration(days: 7));
