...
class ActivityController extends GetxController {
    ...
    if (hour < 9) {
      ...
      if (steps.value < (stepTarget.value * 0.3)) {
        smartSuggestion.value =
            'Còn thiếu hơi nhiều bước chân, đi bộ một chút không? 🚶‍♂️';
      } else {
        smartSuggestion.value =
            'Làm việc năng suất nhé! Đừng quên uống nước. 🥤';
      }
    } else if (hour < 15) {
      ...
      if (steps.value < (stepTarget.value * 0.7)) {
        smartSuggestion.value =
            'Chiều rồi, tranh thủ hoàn thành mục tiêu bước chân nào! 🔥';
      } else {
        smartSuggestion.value =
            '$userPronoun đang làm rất tốt, tiếp tục phát huy nhé! ✨';
      }
    } else {
      smartSuggestion.value =
          'Sắp đến giờ đi ngủ rồi, thư giãn đầu óc thôi. 🌙';
    }
  }

  void _calculateCompletionScore() {
    try {
      double stepRatio = min(
        1.0,
        steps.value / (stepTarget.value > 0 ? stepTarget.value : 8000),
      );
      double waterRatio = min(
        1.0,
        water.value / (waterTarget.value > 0 ? waterTarget.value : 2.0),
      );
      double burnRatio = min(
        1.0,
        calories.value / (calorieTarget.value > 0 ? calorieTarget.value : 2500),
      );
      ...
      _gamification.checkOverachiever(
        calories.value,
      ...
      _gamification.checkTimeBasedAchievements(
        'steps',
      ...
      int uniqueCount =
          (authCtrl.userData['completed_exercise_ids'] as List?)?.length ?? 0;
  ...
  Future<void> updateTargets({
    int? newStepTarget,
    double? newWaterTarget,
    int? newCalorieTarget,
  }) async {
    if (uid.isEmpty) return;
    try {
    ...
    try {
      ...
      if (doc.exists && doc.data() != null) {
        ...
        workoutCalories.value =
            (data['workoutCalories'] as num?)?.toDouble() ?? 0.0;
        activeCalories.value =
            (data['activeCalories'] as num?)?.toDouble() ?? 0.0;
  ...
  Future<void> _fetchWeeklyActivities() async {
    ...
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        double sTarget = stepTarget.value > 0
            ? stepTarget.value.toDouble()
            : 8000;
        double wTarget = waterTarget.value > 0 ? waterTarget.value : 2.0;
        double cTarget = calorieTarget.value > 0
            ? calorieTarget.value.toDouble()
            : 2500;
  ...
  Future<void> manualSync() async {
    ...
    Get.snackbar(
      'Đang đồng bộ',
      'App đang kéo dữ liệu từ Health Connect nhen...',
    ...
    if (steps.value > 0) {
      Get.snackbar(
        'Thành công',
        'Đã đồng bộ ${steps.value} bước chân!',
      ...
      Get.snackbar(
        'Thông báo',
        'Không tìm thấy dữ liệu mới từ Health Connect. $userPronoun kiểm tra lại app Fit nhen!',
  ...
  Future<void> syncWithGoogleFit({bool forceRequestPermissions = false}) async {
    ...
    try {
      ...
      int? dailySteps = await _healthService.health.getTotalStepsInInterval(
        startOfDay,
        now,
      );
          ...
          if (point.value is NumericHealthValue) {
            ...
            if (point.type == HealthDataType.WATER) {
              ...
              if (point.dateTo.isAfter(now)) {
                double totalMin = point.dateTo
                    .difference(point.dateFrom)
                    .inMinutes
                    .toDouble();
                double elapsedMin = now
                    .difference(point.dateFrom)
                    .inMinutes
                    .toDouble();
                if (totalMin > 0) {
      ...
      calories.value =
          activeCalories.value + bmrCalories.value + workoutCalories.value;
      updateMoveMinutes();
      ...
      _calculateCompletionScore();
      debugPrint(
        "--- ActivityController: Đồng bộ XONG! Steps: ${steps.value} ---",
      );
    } catch (e) {
  ...
  Future<void> _updateStreak() async {
    ...
    DocumentReference streakRef = _firestore
        .collection('users')
        .doc(uid)
        .collection('streaks')
        .doc('current');
    ...
    await streakRef.set({'count': currentStreak, 'lastDate': todayStr});
    ...
    if (Get.isRegistered<WorkoutController>()) {
      ...
      for (var schedule in workoutCtrl.todaySchedules) {
        if (schedule['isCompleted'] == true) {
          ...
          final combo = workoutCtrl.combos.firstWhereOrNull(
            (c) => c.title == name,
          );
          if (combo != null) {
            for (var exId in combo.exerciseIds) {
              final exercise = workoutCtrl.allExercises.firstWhereOrNull(
                (e) => e.id == exId,
              );
              double factor = 1.0;
              if (exercise != null) {
              ...
              double exSecs =
                  ((sets * reps * 2.5) + ((sets - 1) * rest)) * factor;
