...
class GamificationService extends GetxService {
  ...
  Map<String, int> getExpRangeForLevel(int level) {
    ...
    return {'min': currentLevelExp, 'max': nextLevelExp};
  }
  ...
  Future<void> awardExp(
    ...
    if (milestoneKey != null) {
      ...
      Map<String, dynamic> status = snap.exists
  ...
  void _checkThemeUnlocks(int exp) {
    if (exp >= 5000) {
      Get.snackbar(
        '🎨 THEME MỚI',
        'Đã mở khóa theme "Pastel Dream" & "Ocean Blue"! Check trong Profile nhen.',
      ...
      Get.snackbar(
        ...
        'Đã mở khóa Dark Mode Neon cực chất!',
  ...
  Future<void> unlockAchievement(
    ...
    if (userEmail.isNotEmpty) {
      MailService.sendAchievementEmail(
        userEmail,
        userName,
        name,
        desc,
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: LinearGradient(
              colors: [Colors.white, Color(0xFFFDEBFF)],
  ...
  Future<void> updateUserStats(
    ...
    if (increment) {
      ...
      Get.find<AuthController>().userData[key] =
          (Get.find<AuthController>().userData[key] ?? 0) + value;
    } else {
  ...
  Future<void> checkWaterMilestones(double water, double target) async {
    if (water >= target * 1.2) {
      await awardExp(
        5,
        'Vượt rào thành công! Thủy thần là đây chứ đâu! 🌊',
        milestoneKey: 'water_120',
      );
    } else if (water >= target) {
      await awardExp(
        15,
        'Tuyệt vời! Đã uống đủ ${target.toStringAsFixed(1)}L nước hôm nay! 🏆',
        milestoneKey: 'water_100',
      );
    } else if (water >= target * 0.5) {
      await awardExp(
        5,
        'Đạt 50% mục tiêu uống nước! Cố lên $userPronoun ơi! 💧',
        milestoneKey: 'water_50',
      );
    }
  }
  ...
  Future<void> checkStepMilestones(int steps, int target) async {
    if (steps >= target * 1.5) {
      await awardExp(
        15,
        'Kẻ cuồng chân! $userPronoun đi quá hăng hái luôn! 🔥',
        milestoneKey: 'steps_150',
      );
    } else if (steps >= target) {
      await awardExp(
        20,
        'Đạt chuẩn y tế rồi! Đôi chân vàng trong làng đi bộ! 🥇',
        milestoneKey: 'steps_target',
      );
    } else if (steps >= 3000) {
      await awardExp(
        5,
        'Đi dạo sương sương 3,000 bước. Khởi đầu tốt đó! 🚶‍♂️',
        milestoneKey: 'steps_3000',
      );
    }
  }
  ...
  Future<void> checkCalorieBurnMilestones(double burned) async {
    if (burned >= 500) {
      await awardExp(
        50,
        'Đốt trên 500 kcal! Một ngày tập luyện cực kỳ bạo chúa! 🔥🦾',
        milestoneKey: 'burn_500',
      );
    } else if (burned >= 300) {
      await awardExp(
        30,
        'Đốt được hơn 300 kcal rồi nè. Mồ hôi rơi là EXP tới! 💦',
        milestoneKey: 'burn_300',
      );
    } else if (burned >= 100) {
      await awardExp(
        10,
        'Cán mốc 100 kcal tiêu thụ. Tiếp tục phát huy nha! ⚡',
        milestoneKey: 'burn_100',
      );
    }
  }

  Future<void> checkNutritionMilestones(
    int mealCount,
    double intake,
    double target,
  ) async {
    if (mealCount >= 3) {
      await awardExp(
        15,
        'Đã ghi nhận đủ 3 bữa chính trong ngày. Ăn uống điều độ quá nè! 🥗',
        milestoneKey: 'meals_3',
      );
    }
    ...
    if (intake >= target * 0.9 && intake <= target * 1.1 && intake > 0) {
      await awardExp(
        20,
        'Calo nạp vào nằm trong ngưỡng an toàn. Rất kỷ luật! ⚖️',
        milestoneKey: 'safe_calories',
      );
    }
  }
  ...
  Future<void> awardProgramComplete(String programName) async {
    await awardExp(
      150,
      'Hoàn thành trọn vẹn giáo án "$programName"! Đẳng cấp là đây! 🏆🏅',
    );
  }
  ...
  Future<void> checkLifetimeAchievements({
    ...
    if (totalSteps != null && totalSteps >= 1000000) {
      await unlockAchievement(
        'million_steps',
        'Kẻ Lữ Hành Vĩ Đại',
        'Tích lũy tổng cộng 1,000,000 bước chân. Một hành trình phi thường! 👣✨',
        500,
      );
    }
    ...
    if (veggieMeals != null && veggieMeals >= 10) {
      await unlockAchievement(
        'healthy_eater',
        'Người Chơi Hệ Healthy',
        'Ghi nhận 10 bữa ăn đầy rau xanh và trái cây. 🥗🍎',
        100,
      );
    }
    ...
    if (totalWater != null && totalWater >= 100) {
      await unlockAchievement(
        'water_god',
        'Thủy Thần',
        'Tích lũy 100 lít nước uống vào app. Cơ thể bạn đã được thanh lọc hoàn toàn! 🌊👑',
        300,
      );
    }
  }
  ...
  Future<void> checkTimeBasedAchievements(String type, {double? value}) async {
    ...
    if (type == 'steps' &&
      ...
      await unlockAchievement(
        'noon_sprinter',
        'Tốc Biến Trưa Hè',
        'Đạt trên 5,000 bước chân chỉ trong khung giờ nắng nóng nhất! 🔥🚶‍♂️',
        100,
      );
    }
    ...
    if (type == 'wakeup' && hour < 6) {
      await unlockAchievement(
        'early_bird_special',
        'Bình Minh Rực Rỡ',
        'Dậy sớm trước 6h sáng và bắt đầu ngày mới đầy năng lượng! ☀️✨',
        100,
      );
    }
    ...
    if (type == 'late_activity' && hour >= 0 && hour <= 3) {
      await unlockAchievement(
        'midnight_owl',
        'Cú Đêm Lầm Lỡ',
        'Bắt quả tang thức khuya nha! Mau đi ngủ đi để cơ bắp phục hồi! 🦉💤',
        10,
      );
    }
  }
  ...
  Future<void> checkOverachiever(double caloriesBurned, double target) async {
    if (caloriesBurned >= target * 3) {
      await unlockAchievement(
        'overachiever_extreme',
        'Hơi Quá Sức Rồi Đó',
        'Vượt mục tiêu calo luyện tập tới 300%. Cháy rực rỡ nhưng nhớ giữ sức nha! 🔥🦾',
        50,
      );
    }
  }
  ...
  Future<void> checkDesertMode(int daysMissing) async {
    if (daysMissing >= 2) {
      await unlockAchievement(
        'desert_mode',
        'Sa Mạc Lời',
        'Quên log nước suốt 48 giờ. Mau uống một ngụm nước để tưới mát tâm hồn đi! 🌵💧',
        5,
      );
    }
  }
  ...
  Future<void> checkPerfectDay(double score) async {
    if (score >= 10.0) {
      await awardExp(
        50,
        'Ngày hoàn hảo! Đạt điểm 10/10 tuyệt đối cho mọi hoạt động! 🌈⭐',
        milestoneKey: 'perfect_day_bonus',
      );
      await unlockAchievement(
        'perfect_day',
        'Perfect Day (Chuẩn Không Cần Chỉnh)',
        'Đạt điểm completionScore tuyệt đối 10/10!',
        100,
      );
    }
  }
  ...
  Future<void> awardStreakExp(int days) async {
    if (days == 3) {
      ...
      await awardExp(
        100,
        'Chuỗi 7 ngày! Bạn đã tạo thành thói quen rồi đó! 🌟',
      );
      await unlockAchievement(
        'water_streak_7',
        'Thủy Thần Trỗi Dậy',
        'Hoàn thành mục tiêu uống nước liên tục trong 7 ngày. 🌊',
        150,
      );
    } else if (days == 21) {
  ...
  Future<void> checkComplexAchievements(
    ...
    if (caloStreak7) {
      await unlockAchievement(
        'calo_warrior_7',
        'Thánh Cày Calo',
        'Vượt mục tiêu calo liên tiếp trong 7 ngày. Sức mạnh vô biên! 🔥🦾',
        500,
      );
    }
    ...
    if (lazy7) {
      await unlockAchievement(
        'lazy_7',
        'Kẻ Lười Biếng',
        'Bỏ tập liên tiếp 7 ngày. Cơ bắp đang khóc thét đó $userPronoun ơi! 🛋️💤',
        5,
      );
    }
    ...
    if (last7Days.length >= 8) {
      ...
      if (scary8) {
        await unlockAchievement(
          'scary_8',
          'Ôi đáng sợ quá!',
          'Hoàn thành vượt định mức TẤT CẢ mục tiêu liên tục 8 ngày. Quái vật giới fitness! 👹💥',
          1000,
        );
      }
    }
  ...
  Future<void> checkNutritionBinge(int mealCount) async {
    if (mealCount >= 5) {
      await unlockAchievement(
        'glutton_5',
        'Kẻ Phàm Ăn',
        'Ăn tới 5 bữa một ngày. Dạ dày của $userPronoun làm bằng thép sao? 🍔🍕🍗',
        50,
      );
    }
  }
  ...
  Future<void> checkForgetfulness(int mealCount, double water) async {
    if (mealCount == 0 && water == 0) {
      await unlockAchievement(
        'thirsty_hungry',
        'Kẻ Đói Khát',
        'Quên cả ăn lẫn uống suốt một ngày. Đừng để tâm hồn héo úa nhen! 🏜️💀',
        5,
      );
    }
  }

  Future<void> checkUltimateAchievement(
    ...
    if (uniqueExercisesCount >= 1300 && streak >= 3) {
      await unlockAchievement(
        ...
        'Trải nghiệm hết 1300+ bài tập và giữ chuỗi phong độ cực cao. Huyền thoại của các huyền thoại! 👑🌌',
        5000,
      );
    }
  }

  Future<void> awardSleepExp(
    double duration,
    double target,
    bool moodLogged,
    bool noSnooze,
    ...
    if (duration >= target) {
      await awardExp(
        20,
        'Ngủ đủ mục tiêu $target tiếng. Phục hồi năng lượng thôi! 😴',
        milestoneKey: 'sleep_target',
      );
    }
    if (moodLogged) {
      await awardExp(
        5,
        'Đã ghi nhận cảm xúc. $userPronoun quan tâm bản thân quá nè! 😊',
        milestoneKey: 'mood_logged',
      );
    }
    if (noSnooze) {
      await awardExp(
        10,
        'Dậy đúng giờ báo thức, không ngủ nướng! Quá kỷ luật! ⏰✨',
        milestoneKey: 'no_snooze',
      );
    }
  }
}
