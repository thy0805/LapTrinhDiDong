...
class WorkoutManagementController extends GetxController {
  ...
  String translate(String key) =>
      _interfaceTranslations[currentLanguage.value]?[key] ?? key;
  ...
  void fetchExercises() {
    ...
    _exerciseSubscription = _firestore
        ...
        .listen((snapshot) {
          allExercises.value = snapshot.docs.map((doc) {
            ...
            String title =
                ...
                data['title'] ??
                data['displayName'] ??
                (currentLanguage.value == 'vi' ? 'Chưa có tên' : 'No Name');
            String image =
                data['gifUrl'] ??
                data['gif_url'] ??
                data['url'] ??
            ...
            if (data['bodyParts'] != null &&
              ...
              category =
                  data['category'] ??
                  (currentLanguage.value == 'vi' ? 'Khác' : 'Other');
            }
            ...
            return ExerciseItem(
              ...
              difficulty:
                  data['difficulty'] ??
                  (currentLanguage.value == 'vi' ? 'Dễ' : 'Easy'),
  ...
  List<ExerciseItem> get filteredExercises {
    ...
    return allExercises.where((ex) {
      bool matchesSearch = ex.title.toLowerCase().contains(
  ...
  Future<void> addExercise(ExerciseItem ex) async {
    try {
      ...
      List<String> instList = ex.description
          .split('\n')
